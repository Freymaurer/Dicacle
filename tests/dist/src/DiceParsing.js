...
export function DiceParsingTypes_PreSet$reflection() {
    return record_type("DiceParsing.DiceParsingTypes.PreSet", [], DiceParsingTypes_PreSet, () => [["setCount", int32_type], ["diceRolls", string_type]]);
}
...
export function DiceParsingTypes_PreDiceRoll$reflection() {
    return record_type("DiceParsing.DiceParsingTypes.PreDiceRoll", [], DiceParsingTypes_PreDiceRoll, () => [["command", Command$reflection()], ["diceRoll", string_type]]);
}
...
export function DiceParsingAux_parseDiceRoll(preDiceRoll) {
    ...
    if (m != null) {
        ...
        if (diceSize === 0) {
            return Dice_create_458B7DF0(diceCount, diceSize);
        }
        else {
            const diceRollParams = remove(input, 0, m[0].length).trim();
            return Dice_create_458B7DF0(diceCount, diceSize, preDiceRoll.command, Pattern_tryExplode(diceRollParams), Pattern_tryReroll(diceRollParams), Pattern_tryKeepDrop(diceRollParams));
        }
    }
    else {
